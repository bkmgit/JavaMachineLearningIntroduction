/*
 * Numerical methods implementations for Neural network
 *
 *  @author: Deus Jeraldy
...
public class np {
    ...
    public static double[][] sigmoid(double[][] a) {
        ...
        for (int j = 0; j < m; j++) {
            for (int i = 0; i < n; i++) {
                z[j][i] = ( 1.0 / ( 1.0 + Math.exp(-1.0*a[j][i])));
            }
        }
        return z;
    }
    ...
    public static double cross_entropy(int batch_size, double[][] Y, double[][] A) {
        ...
        for (int j = 0; j < m; j++) {
            for (int i = 0; i < n; i++) {
                z[j][i] = (Y[j][i] * Math.log(A[j][i])) + ((1 - Y[j][i]) * Math.log(1 - A[j][i]));
            }
        }
     ...
     *
     * @param z matrix
     * @return zout = exp(z) ./ sum(sum(exp(z)))
     */
    public static double[][] softmax(double[][] z) {
        int m = z.length;
        int n = z[0].length;

        double[][] zout = new double[m][n];
	double sum = 0;
        for (int j = 0; j < m; j++) {
            for (int i = 0; i < n; i++) {
                sum += Math.exp(z[j][i]);
